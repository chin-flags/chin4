...
function BlogPostCard({ post }: BlogPostCardProps) {
  return (
    ...
    >
      <Link href={`/chintel/${post.slug}`} className="space-y-4 block">
        <div className="space-y-2">
          <div className="flex justify-between items-start">
            ...
            <span className="text-sm font-medium px-2 py-1 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400">
              {post.category}
            </span>
          </div>
          {post.date && (
...
export default function BlogPosts({ posts }: { posts: BlogPost[] }) {
  const categories: (BlogCategory | 'All')[] = ['All', 'Intel', 'Tales', 'Bad Ideas'];
          ...
          <TabsTrigger 
            ...
            className={`min-w-[100px] px-4 py-2 text-lg font-bold transition-all ${
              category === 'Intel' 
                ...
                : category === 'Tales'
                ? 'data-[state=active]:bg-purple-500 data-[state=active]:text-white hover:text-purple-600'
                : category === 'Bad Ideas'
                ? 'data-[state=active]:bg-pink-500 data-[state=active]:text-white hover:text-pink-600'
                : 'data-[state=active]:bg-emerald-500 data-[state=active]:text-white hover:text-emerald-600'
            }`}
          >
            {category}
          </TabsTrigger>
